...
## Find and exclude parameter combinations

- Aim: avoid overall good fit with bad fit(s) in individual soil depth layers

- Find and excludes parameter combinations with poor fit in individual layers (i.e. by Depth)

- Start with Crop x Stage x Depth combinations (i.e. statDF_cropStageDepth index)

- Criteria of exclusion: r_rmse of layer is > upper quartile r_rmse of all simulations

- <span style="color:red">Is this a good criteria?</span>
...
# Find the 75% percentile for each crop. Upper quartile is the 4th element of the quantile function output
...
# Find & exclude any parameter combinationsimulations with "individual" soil depth layers with r_rmse > upper quartile of r_rmse
remove_badLayer_byCrop <- statDF_cropStageDepth %>%
  rowwise() %>%
  filter(ifelse(Crop == "Lucerne", r_rmse>cutByLayer_luc, 
...
- Treat crops and stages separatelly. Excludes bad parameter combinations within stages

- <span style="color:red">FIXME: Can I use only one of this two options?</span>
...
- Aim: Decide which stats to use as criteria to select optimum parameters
- Many stats are "highly" correlated (e.g. see nse x r_rmse x rmse)
- Idea is to combine stats that have low correlation with each other
- Have chosen <span style="color:red">r_rmse and LC</span> at the moment
- <span style="color:red">Is this a good rationale? Alternatives?</span>

```{r AnalyseStats, fig.height=18,fig.width=12, echo=FALSE, warning=FALSE}
...
- Option 1: filter out sims with bad fit in individual layers 
- Option 2: keep all simulations 
...
# No pre-exclusion of layers with bad fit
df_selected <- statDF_cropStage
...
data_bf %>%
  ggplot(aes(x=lc, y=r_rmse)) + # not working yet
...
#  geom_path(aes(x,y), data=dat) +
  ...
  geom_hline(data=bound_bf, aes(yintercept=r_rmse_best), linetype=2) + #
  geom_vline(data=bound_bf, aes(xintercept=lc_best), linetype=2) # 
...
data_bf %>%
 filter(range_idx == 1) %>%
  ggplot(aes(x=as.numeric(as.character(SurfaceKl)))) +
    geom_histogram() +
    facet_wrap(Stage~Crop, scales= "free") 
