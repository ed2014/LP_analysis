...
```{r PlotSimPop, fig.height=5,fig.width=10, echo=FALSE, warning=FALSE}
statDF_crop %>%
  ggplot(aes(y=lc, x=r_rmse)) + # not working yet
  geom_point(alpha=0.5, aes(size=factor(SurfaceKl),
...
percSel <- 0.05 # percentile of simulations to be selected (Attention to index criteria)

# finds lowest r_rmse for the establishment phase first
bestFit_sims <- statDF_crop  %>%
...
#  filter(rsr == min(rsr) ) %>%   # get smallest error
  filter(row_number() == min(row_number())) # FIXME: gets lowest param value if stats are the same 

#write.csv(bestFit_crop, "testStatsBias.csv")

# only for single selection
 rownames(bestFit_sims) <- paste0(bestFit_sims$Crop,"_",bestFit_sims$Stage,"_",bestFit_sims$Irrigation) 
# 
  t(bestFit_sims) %>%
...
- We can look at independence of residuals
- This will be quantitativelly picked by bias-stats indexes I'd believe (Check with Esther)
...
# Plot "best" parameter choice against all simulations
