...
summarise(Perc95CI = sum(Within_CI95)/n()*100, ObsNo = n()) %>%
  ggplot(aes(x=Stage, y=Perc95CI)) +
  geom_boxplot(alpha=0.2) +
  geom_jitter(width=0.5, aes(colour=factor(SurfaceKl))) +
...
- Start from highest agregation level (best parameter combination per crop)
- i.e. across all depths, treatments, stages etc

```{r DoStatsCrop, echo=TRUE, include=TRUE}

# Separate Crop only
statDF_crop <- dataWork  %>%
  filter(Irrigation == "No Irrigation") %>%  # test optmise for dryland
# filter(Irrigation != "No Irrigation") %>% 
...
#  filter(r_rmse == min(r_rmse) ) %>%   # get smallest error
    filter(r_rmse == min(r_rmse) ) %>%   # get smallest error
#  filter(lc > quantile(lc, 0.95) | rmse < quantile(rmse, 0.05))
...
#  filter(rsr == min(rsr) ) %>%   # get smallest error
  filter(row_number()  == min(row_number())) # gets first set of indexes if still multiple choices - FIXME

#write.csv(bestFit_crop, "testStatsBias.csv")

rownames(bestFit_crop) <- paste0(bestFit_crop$Crop)

 t(bestFit_crop) %>%
   kable(format = "markdown", align = 'c')

 selecParamsEst <- as.character(unique(bestFit_crop$index_crop))
 RVF_luc_est <- bestFit_crop[bestFit_crop$Crop=="Lucerne",]$RootFrontVelocity
 RVF_rye_est <- bestFit_crop[bestFit_crop$Crop=="Ryegrass",]$RootFrontVelocity
