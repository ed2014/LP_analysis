...
```{r XYgraph , fig.height=10,fig.width=12, echo=FALSE, include = FALSE}
dataWork %>%
...
# Add mock confidence limit of observed to enable calculation testing
# FIXME: This will come at high level in raw data later - chunk to be deleted
...
- Mock values of 95%CI until we get actual data from Hamish
...
```{r,fig.width=10, fig.height=15, include = FALSE}
...
simsWithBestFitForDry  %>%
  ...
  scale_x_date(labels = date_format("%b")) + scale_shape(solid = FALSE) + 
  geom_vline(xintercept = as.numeric(simsWithBestFitForDry $Date[estLoc]),linetype = 2) +
  ylab("Volumetric soil moisture (mm)") +
  theme(text = element_text(size=16)) 
```

## How are stats for all irrigation treatmsnts when we use dryland optimum?
...
# print table of stats across all tretamenys
...
## How are statistics across irrigation treatments and depths 
...
#  filter(Crop == "Lucerne") %>%
  ...
  coord_flip() + 
  labs(x = "Soil depth (cm)", y = "Mean square error ((mm/mm))MSE)") +
...
```{r}
#Preparation for big graph

# do Agregated stats .... i.e. best param combination for "crops" across irrigation and stages
...
# Define ranges of good fit

- Creates a flag for simulations that are within ranges of optimum 

```{r}

# Across all simulations (crop/irrig/stage) flag that have the param comb of the best fit for dry crops
flag_best <- statDF_crop_stage_irrig %>%
  mutate(BestFit = ifelse(index_crop %in% 
                            bestFit_dry$index_crop, # get param combination for best fit for dry
                          "Best", "NotBest")) %>%
  mutate(BestFit = factor(BestFit, levels=c("NotBest", "Best"))) 

#head(temp_data_bf[,])

# calculate ranges of acceptable fit +- % of best stats for dry
rangeBestFit <- 0.001 # % around best fit that is considered good enough (subjective equifinality)

bound_best <- flag_best %>%
  filter(BestFit == "Best") %>%
  group_by(Crop) %>%
...
# Create a flag to point param combinations that have r_rmse and lc within range of best fit for dry
flag_best_range <- merge(flag_best,bound_best, by=c("Crop"))

data_bf <- flag_best_range %>%
  rowwise() %>%
...
#  facet_wrap(Irrigation~Crop+Stage, scales = "free") +
 ...
 # ylim(min(statDF_crop$lc),max(statDF_crop$lc))+
 # geom_vline(aes(xintercept=0), linetype=2) + xlab("Percent Bias (%) ") + ylab("RMSE-observations standard deviation (RSR, %)")
  xlab("Lack of correlation (LC, % MSE) ") + ylab(" Relative RMSE (% observed mean)") +
    theme(text = element_text(size=18)) +
    labs(shape="Root Front Velocity (RFV)", 
         colour="Surface kl (kls)", 
         size="kl decay (kld)", linetype="Best fit +-1%")
   # +
```

# Plot overall population of simulations

- Show model performance (i.e. r_rmse and lc) for each different parameter combination
- Huge range of individua model perfirmance
...
statDF_crop_stage_irrig %>%
  ...
  geom_point(alpha=0.8, aes(size=factor(KLReduction),
                 ...
                 shape=factor(RootFrontVelocity))) +
  scale_shape_discrete(solid=F) +
  facet_wrap(Irrigation~Crop+Stage, scales = "free",ncol=4) +
  xlab("Lack of correlation (LC, % MSE) ") + ylab(" Relative RMSE (% observed mean)") +
...
```{r PlotSimPop2, fig.height=10,fig.width=15, echo=FALSE, warning=FALSE, include=FALSE}

# NOT USED - similar results as rRMSE and LC - not excactly but very close 
