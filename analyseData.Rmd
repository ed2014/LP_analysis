...
obsPredFile <- "ObsPreSD15.csv" # uses the k0 (maximum value of kl) at 15 cm depth 
#obsPredFile <- "ObsPreSD30.csv" # We assumed k0 at 30 cm is too severe for crops such as ryegrass
...
dataWork <- dataRaw_merge %>% 
  filter(SurfaceKl != "20") %>% # revove unrealistic parameter levels from test set
  mutate(Date = as.Date(ymd(Date)),
        ...
        Depth = as.character(gsub("D", "",Depth)),#  FIXME: not sure why does not work all in one line
...
# reduce parameter space to 5 levels
...
#ggsave(file="Fig_TestFit.tiff", dpi = 300)
g
# Save
# saveDir <- "\\\\Lindfs12\\home$\\cfleit\\My Documents\\My papers\\2013-LucernePasture\\kl\\submitted\\round_1"
 ggsave(file="Fig_8_FitTest.tiff", dpi = 300, path=saveDir, g)
...
# Save
 ...
 ggsave(file="Fig_9_SensAnBar.tiff", dpi = 300, path=saveDir, g)
...
# Save
# saveDir <- "\\\\Lindfs12\\home$\\cfleit\\My Documents\\My papers\\2013-LucernePasture\\kl\\submitted\\round_1"
 ggsave(file="Fig_10_Equif.tiff", dpi = 300, path=saveDir, g)
