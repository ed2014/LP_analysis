...
obsPredFile <- "ObsPreSD15.csv"
#obsPredFile <- "ObsPreSD30.csv" # We assumed this is too severe for crops such as ryegrass
...
## Check averages - calculated and raw from Hamish's run
```{r, include=FALSE, echo=FALSE}
plot(dataWork$Obs_av,dataWork$Obs)
```
...
# for the best fit dry (FIXME: confude here - how to get stats for best fit across both est/reg)
# tableCI95 <- within95CI_df %>%
#   ungroup() %>%
#   dplyr::select(Crop:index_crop_stage,Within_CI95) %>%
#   filter(index_crop %in% unique(bestFit_dry$index_crop)) %>% # select best param sims only
#   group_by(Crop, Stage,RootFrontVelocity,KLReduction,SurfaceKl, index_crop_stage) %>% # agregate by crop
...
df <- gauch_depth %>%
  mutate(mse = rmse^2, sb=(sb/100*mse)/meanObs*100,nu=(nu/100*mse)/meanObs*100,lc=(lc/100*mse)/meanObs*100) %>% 
 # FIXME: What's the best stats to use here?
...
#  filter(Crop == "Lucerne") %>%
  ...
  facet_grid(Irrigation~Crop+Stage,scales = "free") +
  coord_flip() + 
  labs(x = "Soil depth (cm)", y = "Relative mean square error (% observed mean)") +
