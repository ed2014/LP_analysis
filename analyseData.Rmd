...
dataWork$Stage <- as.factor(ifelse(dataWork$Date<="2012-06-01 00:00:00","Establishment","Regrowth"))
...
dataWork$index_crop <- paste0(dataWork$Crop, dataWork$SurfaceKl, dataWork$RootDepth, dataWork$KLReduction, dataWork$RootFrontVelocity)
dataWork$index_stage <- paste0(dataWork$Crop, dataWork$Stage, dataWork$SurfaceKl, dataWork$RootDepth, dataWork$KLReduction, dataWork$RootFrontVelocity)
...
st1 <- dataWork %>%
  dplyr::group_by(Crop, Stage, SurfaceKl, RootDepth, KLReduction, RootFrontVelocity) %>%
  dplyr::summarise(
    n = n(),
...
st2 <- dataWork %>%
  group_by(Crop, SurfaceKl, RootDepth, KLReduction, RootFrontVelocity) %>%
  summarise(
    n = n(),
    r2 = round(br2(Predicted,Observed)*100,0),
    rmse = round(rmse(Predicted,Observed),3),
...
XY graph for Prediced x Observed for Crops and Stages
...
Search for the best fit (lowest r_rmse)
...
```{r PlotBestFits, fig.height=18,fig.width=12, echo=FALSE}
...
sel1 <- dataWork %>%
  filter(
    ...
    RootDepth == "2000" &
    KLReduction == ".0.0005" &
    RootFrontVelocity == "15"
  ) 
                             
sel1 %>%
   ggplot() +
  geom_point(aes(x=Date, y=Observed, colour="darkblue"), size=3) +
