...
#  summarise(rainMean = mean(rain), rainSd = sd(rain)/2) %>%
  ...
  #           aes(x=date, y=rain), colour="black") +
    ...
    #         aes(x=Date, y=rain*30.5), colour="blue", alpha=0.3) +
      # geom_line(data=rain.month.hyst, 
   geom_line(data=rain.month.hyst, aes(x=date, y=ep), colour="black", alpha=0.3, linetype=2) +
   geom_line(data=rain.month.hyst, aes(x=date, y=rain), colour="black", linetype=1) +
