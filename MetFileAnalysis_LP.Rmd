...
Append y similar seasons of histirical etp
...
#  summarise(rainMean = mean(rain), rainSd = sd(rain)/2) %>%
  ...
  #           aes(x=date, y=rain), colour="black") +
    ...
    #         aes(x=Date, y=rain*30.5), colour="blue", alpha=0.3) +
      geom_line(data=etp_day_av.hist, 
            aes(x=Date, y=eo*30.5), colour="black", alpha=0.3, linetype=2) +
 # theme_bw() +
  xlab("Date") + 
...
# Save
 ...
 ggsave(file="Fig_1_weather.tiff", dpi = 300, path=round1Dir, g)
