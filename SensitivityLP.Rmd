...
si_df_final %>%
  tidyr::gather("Si_Item","Si_Value", SurfaceKl:Interactions) %>%
  mutate(Si_Item = factor(Si_Item,
  levels = c("SurfaceKl","KLReduction","RootFrontVelocity","Interactions"),
  labels = c(expression("kl"["s"]),expression(lambda["kl"]),"RFV", "Interactions") # FIXME: synthax to use expressions here?
                          )) %>%
  ggplot(aes(x=rev(thisIrrig), y=Si_Value, fill=factor(Si_Item))) +
  geom_bar(stat="identity") +
